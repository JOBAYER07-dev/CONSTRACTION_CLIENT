...
export default function ProfilePage() {
  ...
  // ✅ Fix — name/image ke useState initial value hisebe na, session-based derived value hisebe track korbo
  useEffect(() => {
    ...
    const fetchCount = async () => {
      try {
        ...
        if (response?.success && Array.isArray(response.data)) {
          const myCount = response.data.filter(
  ...
  // session load hobar por, form field-gulo ekbar-i sync koro — key diye remount trick use kore effect-e setState lagbe na
  const displayName = newName || session?.user?.name || '';
  const displayImage = imagePreview || session?.user?.image || '';
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newName.trim())
      return toast.error('Identity Name fields cannot be empty!');
    ...
    return (
      <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center p-6 text-center">
        ...
        <button
          ...
          className="bg-[#10B981] text-white px-6 py-2.5 rounded-xl font-medium hover:bg-[#059669] transition"
        >
          Go to Login
        </button>
      </div>
      ...
      >
        <div className="text-center md:text-left mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">
            Manage your professional credentials and structural metrics.
          </p>
        </div>
          ...
          <div className="relative group">
            <div className="w-28 h-28 rounded-2xl overflow-hidden border-2 border-slate-700 p-1 group-hover:border-[#10B981] transition-colors duration-300 shadow-xl bg-slate-900">
              <img
                src={
                  session.user.image ||
                  'https://api.dicebear.com/7.x/bottts/svg?seed=Jobayer'
                }
                alt="Avatar"
        ...
        >
          <div className="space-y-1">
            <h4 className="text-slate-400 text-sm font-medium">
              Total AI Estimations
            </h4>
            <p className="text-xs text-slate-500">
              Number of structural models processed by you.
            </p>
          </div>
