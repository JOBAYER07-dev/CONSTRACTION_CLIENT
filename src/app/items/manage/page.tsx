...
export default function ManageProjectsPage() {
  ...
  const { data: projects = [], isLoading } = useQuery({
    queryKey: ['myProjects', session?.user?.id],
    ...
    enabled: !isAuthPending && !!session?.user,
  });
  ...
  const filteredProjects = projects.filter(project => {
    ...
    const matchesType =
      selectedType === 'All' || project.buildingType === selectedType;
    ...
    return (
      <div className="min-h-screen bg-[#020617] text-slate-200 flex flex-col items-center justify-center p-6">
        <p className="text-rose-400 mb-4">
          Please log in to manage your construction projects.
        </p>
        <Link
          href="/login"
          className="bg-[#10B981] text-white px-6 py-2.5 rounded-lg font-medium hover:bg-[#059669] transition"
        >
          Go to Login
        </Link>
      </div>
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            View, edit, and organize your construction projects.
          </p>
        </div>
        <Link
          href="/items/add"
          className="inline-flex items-center gap-2 bg-[#10B981] hover:bg-[#059669] text-white px-5 py-2.5 rounded-lg font-medium transition-all shadow-[0_0_15px_rgba(16,185,129,0.3)]"
        >
          <PlusIcon />
          ...
          {!isLoading && filteredProjects.length > 0 && (
            <div className="bg-slate-900/30 px-6 py-4 border-t border-slate-800 flex items-center justify-between text-sm">
              <div className="text-slate-400">
                ...
                to{' '}
                <span className="text-slate-200 font-medium">
                  {indexOfLastItem > filteredProjects.length
          ...
          <div className="bg-[#0F172A] border border-slate-800 rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <div className="flex items-center gap-3 justify-end">
              <button
                ...
                className="px-4 py-2 rounded-lg text-slate-300 hover:bg-slate-800 hover:text-white transition-colors font-medium cursor-pointer disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={deleteMutation.isPending}
                className="px-4 py-2 rounded-lg bg-rose-500 hover:bg-rose-600 text-white transition-colors shadow-[0_0_15px_rgba(244,63,94,0.3)] font-medium cursor-pointer disabled:opacity-50"
              >
